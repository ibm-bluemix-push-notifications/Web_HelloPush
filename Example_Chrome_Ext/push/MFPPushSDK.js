...
function MFPPush(){
  ...
  */
  ...
  this.initializeApp = function(params, callback ) {
    printResults("started initialize");
    _isExtension = true;
    _appId = params.appGUID ? params.appGUID : "";
    _appRegion = params.appRegion ? params.appRegion : "";
  ...
  /**
  * Registers the website on to the IMFPush Notification Server
  *
  * @param userId: the User ID value. -- optional
  */
  this.register = function (userId, callbackM){
    registerPush(userId,callbackM,false)
  };

  /**
  * Registers the extension or app on to the IMFPush Notification Server
  *
  * @param userId: the User ID value. -- optional
  */
  this.registerApp = function (userId, callbackM){
    registerPush(userId,callbackM,true)
  };
  ...
  */
  this.unRegisterDevice = function(callbackM){
    ...
    */
    ...
    function registerPush(userId,callbackM,isExtension) {
      ...
      if(!isExtension){
        ...
        navigator.serviceWorker.ready.then(function(reg) {
          ...
          loadJSON(function(response) {
            ...
            printResults("sender Id is : ",actual_JSON.gcm_sender_id);
            var senderIds = [actual_JSON.gcm_sender_id];
            chrome.gcm.register(senderIds, function(registrationId){

              if (chrome.runtime.lastError) {
                // When the registration fails, handle the error and retry the
                // registration later.
                MFPPushResponseSet(chrome.runtime.lastError,401,"Error");
                callbackM(MFPPushResponse)
                return;
              }
          ...
          //           var mydata = JSON.parse(manifest);
          // alert(mydata[0].name);
      ...
      function update () {
        ...
        registerPush(_userId, callback,false);
      }
